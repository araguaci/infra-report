#!/usr/bin/env bash
# ============================================================================
# infra-report.sh — Auditoria local de infraestrutura, portas e segurança
# Uso:  sudo bash infra-report.sh
#       sudo bash infra-report.sh --json /tmp/infra-report.json
#       sudo bash infra-report.sh --host exemplo.com
...
# Renderizador no mesmo diretório do script; override: --render-py ou INFRA_REPORT_RENDER
RENDER_PY="${INFRA_REPORT_RENDER:-${SCRIPT_DIR}/infra_report_render.py}"
...
cleanup() {
  [[ -n "${REPORT_TMP:-}" && -d "$REPORT_TMP" ]] && rm -rf "$REPORT_TMP"
}
trap cleanup EXIT
...
Opções:
  ...
  --render-py PATH  Caminho do infra_report_render.py (padrão: ao lado deste script)
  --timeout N       Timeout de probes em segundos (padrão: ${TIMEOUT_SEC})
  -V, --version     Mostra a versão e sai
  -h, --help        Ajuda

Requer: bash 4+, python3 (≥3.8), ss, curl, openssl.
Opcional: docker, ufw, firewall-cmd, fail2ban-client, apt/dnf/apk/zypper.
EOF
}
...
preflight() {
  if [[ -z "${BASH_VERSINFO:-}" ]] || ((BASH_VERSINFO[0] < 4)); then
    echo "ERRO: Bash 4+ é necessário (associative arrays). Atual: ${BASH_VERSION:-desconhecido}" >&2
    exit 1
  fi

  local missing=()
  local cmd
  for cmd in python3 ss curl openssl; do
    need_cmd "$cmd" || missing+=("$cmd")
  done
  if ((${#missing[@]} > 0)); then
    echo "ERRO: comandos obrigatórios ausentes: ${missing[*]}" >&2
    echo "  Debian/Ubuntu: apt-get install -y iproute2 curl openssl python3" >&2
    echo "  RHEL/Rocky:    dnf install -y iproute curl openssl python3" >&2
    echo "  Alpine:        apk add iproute2 curl openssl python3 bash" >&2
    exit 1
  fi

  if [[ ! -f "$RENDER_PY" ]]; then
    echo "ERRO: renderizador não encontrado: $RENDER_PY" >&2
    echo "  Coloque infra_report_render.py no mesmo diretório do script" >&2
    echo "  ou use --render-py /caminho/para/infra_report_render.py" >&2
    exit 1
  fi
}
...
# ─── 4. SSH ───────────────────────────────────────────────────────────────────
# Lista arquivos sshd na ordem de Include (primeiro valor vence, como no OpenSSH).
sshd_config_files() {
  local main="/etc/ssh/sshd_config"
  [[ -f "$main" ]] || return 0
  echo "$main"
  local line raw pattern f
  while IFS= read -r line || [[ -n "$line" ]]; do
    [[ "$line" =~ ^[[:space:]]*# ]] && continue
    echo "$line" | grep -qiE '^[[:space:]]*Include[[:space:]]+' || continue
    raw="$(echo "$line" | sed -E 's/^[[:space:]]*[Ii][Nn][Cc][Ll][Uu][Dd][Ee][[:space:]]+//;s/[[:space:]]+$//;s/^"//;s/"$//')"
    [[ -n "$raw" ]] || continue
...
analyze_ssh() {
  ...
  local ssh_source="files" SSHD_EFFECTIVE=""
  ...
  if need_cmd sshd; then
    ...
    [[ -n "$SSHD_EFFECTIVE" ]] && ssh_source="sshd -T"
  fi
  ...
  get_ssh() {
    ...
    if [[ -n "$SSHD_EFFECTIVE" ]]; then
      echo "$SSHD_EFFECTIVE" | awk -v k="$key_lc" 'tolower($1)==k {print $2; exit}'
      return 0
    fi
    # Fallback: primeiro match nos arquivos (ordem Include), alinhado ao OpenSSH
    local f
    while IFS= read -r f; do
      [[ -f "$f" ]] || continue
      local val
      val="$(grep -Ei "^[[:space:]]*${key}[[:space:]]+" "$f" 2>/dev/null \
        | grep -v '^[[:space:]]*#' | head -n1 | awk '{print $2}' | tr -d '"' || true)"
      if [[ -n "$val" ]]; then
        echo "$val"
        return 0
      fi
...
print(json.dumps({
  ...
  "config_source": sys.argv[6],
}, ensure_ascii=False))
' "$port" "${permit_root}" "${pwd_auth}" "${pubkey}" "$fail2ban" "$ssh_source" >"$REPORT_TMP/ssh.json"
}
...
analyze_docker() {
  ...
  while IFS= read -r cid; do
    [[ -n "$cid" ]] || continue
    insp="$(docker inspect --format '{{.Name}} {{.HostConfig.Privileged}}' "$cid" 2>/dev/null || true)"
    if [[ "$insp" == *" true" ]]; then
      priv+="${insp}"$'\n'
    fi
...
# ─── 7. Misc ──────────────────────────────────────────────────────────────────
count_pkg_updates() {
  # Echo: pkg_mgr upgradable_count auto_updates(true|false|n/a)
  local n=0 mgr="none" auto="n/a"
  ...
  if need_cmd apt-get; then
    ...
    mgr="dnf"
    # exit 100 = updates available; ignore exit status
    n="$(dnf -q check-update 2>/dev/null | awk 'NF && $1 !~ /^Last/ && $1 !~ /^$/ {c++} END{print c+0}' || true)"
    if rpm -q dnf-automatic >/dev/null 2>&1 || systemctl is-enabled --quiet dnf-automatic.timer 2>/dev/null; then
      auto="true"
    else
      auto="false"
    fi
    ...
    mgr="yum"
    n="$(yum -q check-update 2>/dev/null | awk 'NF && $1 !~ /^Load/ && $1 !~ /^$/ {c++} END{print c+0}' || true)"
    auto="false"
  elif need_cmd apk; then
    mgr="apk"
    n="$(apk list -u 2>/dev/null | grep -c . || true)"
    auto="n/a"
  elif need_cmd zypper; then
    mgr="zypper"
    n="$(zypper --non-interactive --quiet list-updates 2>/dev/null | grep -cE '^v \|' || true)"
    if systemctl is-enabled --quiet packagekit-offline-update.service 2>/dev/null \
      || [[ -x /usr/lib/zypp/plugins/services/update-checker ]]; then
      auto="n/a"
    else
      auto="false"
    fi
...
analyze_misc() {
  ...
  local upgradable=0 unattended=false cron_jobs=0 pkg_mgr="none" auto_flag="n/a"

  read -r pkg_mgr upgradable auto_flag <<<"$(count_pkg_updates)"

  if [[ "$pkg_mgr" != "none" ]]; then
    kv "Gerenciador pkgs" "$pkg_mgr"
    kv "Pacotes atualizáveis" "$upgradable"
    if [[ "${upgradable:-0}" -gt 50 ]]; then
      finding WARN "Muitos pacotes pendentes ($upgradable via $pkg_mgr)" \
        "Aplique atualizações de segurança regularmente." "hardening"
    elif [[ "${upgradable:-0}" -gt 0 ]]; then
      finding INFO "$upgradable pacotes com update disponível ($pkg_mgr)" "" "hardening"
    else
      finding OK "Sistema aparentemente atualizado ($pkg_mgr)" "" "hardening"
    fi
  else
    finding INFO "Gerenciador de pacotes não detectado (apt/dnf/yum/apk/zypper)" "" "hardening"
  fi
  ...
  case "$auto_flag" in
    true)
      ...
      if [[ "$pkg_mgr" == "apt" ]]; then
        ...
        finding OK "Atualizações automáticas detectadas ($pkg_mgr)" "" "hardening"
      fi
      ;;
    false)
      if [[ "$pkg_mgr" == "apt" ]]; then
        ...
        finding INFO "dnf-automatic / updates automáticos não detectados" \
          "Considere dnf-automatic ou timer de security updates." "hardening"
      fi
      ;;
  esac
...
print(json.dumps({
  ...
  "upgradable": int(sys.argv[2]),
  "apt_upgradable": int(sys.argv[2]),
  "unattended_upgrades": sys.argv[3]=="true",
  "auto_updates": sys.argv[4],
  "cron_d_jobs": int(sys.argv[5])
}, ensure_ascii=False))
' "$pkg_mgr" "$upgradable" "$unattended" "$auto_flag" "$cron_jobs" >"$REPORT_TMP/hardening.json"
}
